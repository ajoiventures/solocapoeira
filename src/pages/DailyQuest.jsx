...
function FlowSessionCard({ store }) {
...
function DailyBonusChallenge({ store }) {
  const today = new Date().toISOString().split("T")[0];
...
function SessionLogBlock({ xp, onLog }) {
...
// ── BonusSection — outer "Bonus" toggle wrapping extra work + sand session ─
function BonusSection({ allExtras, dailyExtras, checkedBonusItems, store, navigate, ALL_COLORS, ALL_LABELS, sandSession }) {
  const [open, setOpen] = useState(false);
  const [openCats, setOpenCats] = useState({});
  const [openSandSecs, setOpenSandSecs] = useState({});
  const [sandDoneIds, setSandDoneIds] = useState(new Set());

  const extraDone = allExtras.filter((ex) => checkedBonusItems.includes(ex.id)).length;
  const extraXP   = allExtras.reduce((s, e) => s + (e.xp || 0), 0);
  const sandTotalExs = sandSession?.sections?.reduce((s, sec) => s + (sec.exercises?.filter(Boolean).length || 0), 0) || 0;
  const sandDone  = sandDoneIds.size;
  const sandXP    = sandSession ? (sandDone >= sandTotalExs && sandTotalExs > 0 ? sandSession.totalXP : sandDone >= Math.ceil(sandTotalExs * 0.5) ? Math.round(sandSession.totalXP * 0.6) : 0) : 0;
  ...
  function toggleCat(cat) { setOpenCats((p) => ({ ...p, [cat]: !p[cat] })); }
  function toggleSandSec(i) { setOpenSandSecs((p) => ({ ...p, [i]: !p[i] })); }
  function toggleSandEx(id) {
    setSandDoneIds((prev) => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  }
        ...
        <div style={{ border: "1px solid var(--border)", borderTop: "none", borderRadius: "0 0 8px 8px", overflow: "hidden" }}>
          ...
          {["strength", "conditioning", "core", "sand"].map((cat) => {
            ...
            return (
              <div key={cat}>
                ...
                >
                  <span style={{ fontSize: 12, fontWeight: 700, color: catColor, flex: 1 }}>{ALL_LABELS[cat]}</span>
                ...
                {catOpen && (
                  <div style={{ padding: "8px 12px", display: "flex", flexDirection: "column", gap: 6, background: "var(--surface)" }}>
                    {items.map((ex) => {
                      ...
                      if (checked) {
                        return (
                          <div key={ex.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderRadius: 8, background: "rgba(46,140,120,0.06)", border: "1px solid var(--green)" }}>
                            <span style={{ fontSize: 14 }}>{ex.icon}</span>
                            <span style={{ flex: 1, fontSize: 12, fontWeight: 600, color: "var(--green)", textDecoration: "line-through", opacity: 0.75 }}>{ex.label}</span>
                            <span style={{ fontSize: 10, fontWeight: 700, color: "var(--yellow)" }}>+{ex.xp} XP</span>
                            <button onClick={() => store.toggleBonusItem(ex.id, ex.xp)}
                              style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, fontWeight: 700, border: "1px solid var(--green)", background: "rgba(46,140,120,0.15)", color: "var(--green)", cursor: "pointer" }}>✓</button>
                          </div>
                        );
                      }
                      return (
          ...
          {sandSession && sandSession.sections?.length > 0 && (
            <div>
              ...
              >
                <span style={{ fontSize: 12, fontWeight: 700, color: "#d97706" }}>🏖️ Beach Session</span>
                <span style={{
              ...
              {openCats["_sand_session"] && (
                <div style={{ background: "var(--surface)" }}>
                  {sandSession.sections.map((sec, si) => {
                    const secExs = (sec.exercises || []).filter(Boolean);
                    const secDone = secExs.filter((ex) => sandDoneIds.has(ex.id)).length;
                    const secOpen = !!openSandSecs[si];
                    return (
                      <div key={si}>
                        <button
                          onClick={() => toggleSandSec(si)}
                          style={{
                            width: "100%", display: "flex", alignItems: "center", gap: 8,
                            background: "var(--surface2)", padding: "7px 16px", cursor: "pointer", textAlign: "left",
                            border: "none", borderTop: si > 0 ? "1px solid var(--border)" : "none",
                          }}
                        >
                          <span style={{ fontSize: 12 }}>{sec.icon}</span>
                          <span style={{ flex: 1, fontSize: 11, fontWeight: 600, color: "var(--text2)" }}>{sec.label}</span>
                          <span style={{ fontSize: 9, color: "var(--text3)" }}>{sec.duration}</span>
                          <span style={{ fontSize: 10, color: secDone === secExs.length && secDone > 0 ? "var(--green)" : "var(--text3)", fontWeight: 700, marginLeft: 8 }}>
                            {secDone}/{secExs.length}
                          </span>
                        </button>
                        {secOpen && (
                          <div style={{ padding: "6px 12px", display: "flex", flexDirection: "column", gap: 4 }}>
                            {secExs.map((ex) => {
                              const done = sandDoneIds.has(ex.id);
                              return (
                                <div key={ex.id} style={{
                                  ...
                                  <button onClick={() => toggleSandEx(ex.id)}
                                  ...
                                  <span style={{ fontSize: 9, color: "var(--yellow)", fontWeight: 700 }}>+{ex.xp} XP</span>
                                </div>
                              );
                            })}
                          </div>
...
// ── ExtraWorkSection — 2-level collapse ────────────────────────────────────
